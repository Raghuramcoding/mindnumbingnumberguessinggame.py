...
class NumberGuessingGameUI:
    ...
    def show_difficulty_screen(self):
        ...
        btn_easy = tk.Button(self.main_frame, text="Easy (1-25, 20 attempts)", width=30,
                             command=lambda: self.start_game("easy", 25, 20))
        btn_medium = tk.Button(self.main_frame, text="Medium (1-50, 10 attempts)", width=30,
                               command=lambda: self.start_game("medium", 50, 10))
        btn_hard = tk.Button(self.main_frame, text="Hard (1-100, 5 attempts)", width=30,
                             command=lambda: self.start_game("hard", 100, 5))
        btn_exit = tk.Button(self.main_frame, text="Exit", width=30, command=self.root.quit)
